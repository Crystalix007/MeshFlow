...
Quad::Quad(const cv::Point2f& inV00, const cv::Point2f& inV01, const cv::Point2f& inV10,
           const cv::Point2f& inV11) {
	V00.x = inV00.x;
	V00.y = inV00.y;
	V01.x = inV01.x;
	V01.y = inV01.y;
...
bool Quad::getBilinearCoordinates(const cv::Point2f& pt, vector<double>& coefficient) const {
	...
	if (k1 >= -0.999999 && k1 <= 1.000001 && k2 >= -0.999999 && k2 <= 1.000001) {
		...
		// c_y, d_x); 		printf("(%lf, %lf, %lf, %lf) -> (%lf, %lf, %lf)\n", tmp1, tmp2, tmp3, tmp4,
		// bigA, bigB, bigC);
		return false;
	}
...
cv::Point2f Quad::getPointByBilinearCoordinates(const vector<double>& coefficient) const {
	double resx = V00.x * coefficient[0] + V01.x * coefficient[1] + V10.x * coefficient[2] +
	              V11.x * coefficient[3];
	double resy = V00.y * coefficient[0] + V01.y * coefficient[1] + V10.y * coefficient[2] +
	              V11.y * coefficient[3];
...
void Mesh::drawMesh(cv::Mat& targetImg) {
	...
	for (int i = 1; i < height; i++) {
		for (int j = 1; j < width; j++) {
			...
			if (pUp.x > -9999.0 && pUp.y > -9999.0 && pCur.x > -9999.0 && pCur.y > -9999.0) {
				double dis =
				    sqrt((pUp.x - pCur.x) * (pUp.x - pCur.x) + (pUp.y - pCur.y) * (pUp.y - pCur.y));
				// if(dis<100){
				line(temp, cv::Point2f(pUp.x, pUp.y), cv::Point2f(pCur.x, pCur.y), color, lineWidth,
				     cv::LINE_AA);
				//}
			}
			if (pLeft.x > -9999.0 && pLeft.y > -9999.0 && pCur.x > -9999.0 && pCur.y > -9999.0) {
				double dis = sqrt((pLeft.x - pCur.x) * (pLeft.x - pCur.x) +
				                  (pLeft.y - pCur.y) * (pLeft.y - pCur.y));
				// if(dis<100){
				line(temp, cv::Point2f(pLeft.x, pLeft.y), cv::Point2f(pCur.x, pCur.y), color,
				     lineWidth, cv::LINE_AA);
				//}
			}
			cv::circle(temp, cv::Point(pUp.x, pUp.y), lineWidth + 2, cv::Scalar(45, 57, 167), -1);
			cv::circle(temp, cv::Point(pLeft.x, pLeft.y), lineWidth + 2, cv::Scalar(45, 57, 167),
			           -1);
	...
	for (int i = 1; i < height; i++) {
		...
		if (pLeft.x > -9999.0 && pLeft.y > -9999.0 && pLeftUp.x > -9999.0 && pLeftUp.y > -9999.0) {
			double dis = sqrt((pLeft.x - pLeftUp.x) * (pLeft.x - pLeftUp.x) +
			                  (pLeft.y - pLeftUp.y) * (pLeft.y - pLeftUp.y));
			// if(dis<100){
			line(temp, cv::Point2f(pLeft.x, pLeft.y), cv::Point2f(pLeftUp.x, pLeftUp.y), color,
			     lineWidth, cv::LINE_AA);
			//}
		}
		cv::circle(temp, cv::Point(pLeftUp.x, pLeftUp.y), lineWidth + 2, cv::Scalar(45, 57, 167),
		           -1);
	...
	for (int j = 1; j < width; j++) {
		...
		if (pLeftUp.x > -9999.0 && pLeftUp.y > -9999.0 && pUp.x > -9999.0 && pUp.y > -9999.0) {
			double dis = sqrt((pLeftUp.x - pUp.x) * (pLeftUp.x - pUp.x) +
			                  (pLeftUp.y - pUp.y) * (pLeftUp.y - pUp.y));
			// if(dis<100){
			line(temp, cv::Point2f(pLeftUp.x, pLeftUp.y), cv::Point2f(pUp.x, pUp.y), color,
			     lineWidth, cv::LINE_AA);
			//}
		}
		cv::circle(temp, cv::Point(pUp.x, pUp.y), lineWidth + 2, cv::Scalar(45, 57, 167), -1);
...
bool isPointInTriangular(const cv::Point2f& pt, const cv::Point2f& V0, const cv::Point2f& V1,
                         const cv::Point2f& V2) {
	double lambda1 = ((V1.y - V2.y) * (pt.x - V2.x) + (V2.x - V1.x) * (pt.y - V2.y)) /
	                 ((V1.y - V2.y) * (V0.x - V2.x) + (V2.x - V1.x) * (V0.y - V2.y));
	double lambda2 = ((V2.y - V0.y) * (pt.x - V2.x) + (V0.x - V2.x) * (pt.y - V2.y)) /
	                 ((V2.y - V0.y) * (V1.x - V2.x) + (V0.x - V2.x) * (V1.y - V2.y));
	double lambda3 = 1 - lambda1 - lambda2;
	if (lambda1 >= 0.0 && lambda1 <= 1.0 && lambda2 >= 0.0 && lambda2 <= 1.0 && lambda3 >= 0.0 &&
	    lambda3 <= 1.0) {
		return true;
	} else {
		return false;
				...
				if (flag) {
					...
					if (q1.isPointIn(ptInSrc)) {
						for (int c = 0; c < 3; c++) {
							...
							dst.at<cv::Vec3b>(i + gap, j + gap)[c] =
							    (tmpCoe[0] * f0 *
							         src.at<cv::Vec3b>(int(tmpV00.y), int(tmpV00.x))[c] +
							     tmpCoe[1] * f1 *
							         src.at<cv::Vec3b>(int(tmpV01.y), int(tmpV01.x))[c] +
							     tmpCoe[2] * f2 *
							         src.at<cv::Vec3b>(int(tmpV10.y), int(tmpV10.x))[c] +
							     tmpCoe[3] * f3 *
							         src.at<cv::Vec3b>(int(tmpV11.y), int(tmpV11.x))[c]) /
